<?php
		...
		$data = getAllDataFromTable('delegates.*,users.uname as user','delegates','LEFT OUTER JOIN users ON delegates.duid = users.uid ORDER BY delegates.dctime DESC','');
		...
		if($item['dctime'] != '') { if(date('H', $item['dctime']) < 12) $time = ArabicTools::arabicDate($system['calendar'].' Y-m-d h-i-s', $item['dctime']).' '.lang('am'); else $time = ArabicTools::arabicDate($system['calendar'].' Y-m-d h-i-s', $item['dctime']).' '.lang('pm'); }
		else $time = '';
		...
		$mpdf->Output('delegates_report_'.ArabicTools::arabicDate($system['calendar'].' Y-m-d-H-i-s', time()).'.pdf', 'I');
?>